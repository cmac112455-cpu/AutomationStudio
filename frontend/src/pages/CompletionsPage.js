          ...
          <div className="relative max-w-7xl max-h-[90vh] w-full">
            ...
            <div className="flex items-center justify-center" onClick={(e) => e.stopPropagation()}>
              {lightboxMedia.type === 'image' ? (
                <img 
                  src={`data:image/png;base64,${lightboxMedia.data}`}
                  alt={lightboxMedia.prompt}
                  className="max-w-full max-h-[85vh] object-contain"
                />
              ) : (
                <video 
                  src={`data:video/mp4;base64,${lightboxMedia.data}`}
                  controls
                  autoPlay
            ...
            <div className="absolute bottom-4 left-4 right-4 bg-black/70 backdrop-blur rounded-lg p-4">
              <p className="text-white font-semibold">{lightboxMedia.prompt}</p>
              <p className="text-gray-400 text-sm mt-1">
                {lightboxMedia.type === 'image' ? (
                  <>
                    <ImageIcon className="w-4 h-4 inline mr-1" />
                    {lightboxMedia.size || 'Image'}
                  </>
                ) : (
                  <>
                    <VideoIcon className="w-4 h-4 inline mr-1" />
                    {lightboxMedia.size || 'Video'} • {lightboxMedia.duration}s
                  </>
                )}
              </p>
            </div>
